...
class ui
{
  public static function renderEditors($editors)
  {
  ...
  public static function renderSideBar($items, $mobile = false)
  {
    if (!$mobile)
...
function handleJoinSpace($params)
{
  ...
  if ($allowed) {
    // 创建符号链接
    $userMappings = getUserMapping($mappingFile, $_SESSION['user']['id']);
    $currentUserDir = getUserDir($userMappings, $_SESSION['user']['id']);
...
function getUserMappingFile()
{
  if (!file_exists(__DIR__ . '/user')) {
    mkdir(__DIR__ . '/user', 0755, true);
  }
  if (!file_exists(__DIR__ . '/user/' . $GLOBALS['ENCYC'] . '.json')) {
    file_put_contents(__DIR__ . '/user/' . $GLOBALS['ENCYC'] . '.json', json_encode([]));
  }
