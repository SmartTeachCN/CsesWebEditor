...
function confirm(message, callback, args) {
  ...
  modal.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      confirmAction(true, modal, callback, args);
    } else if (event.key === "Escape") {
      confirmAction(false, modal, callback, args);
    }
  });
}
...
function prompt(message, callback, args) {
  const modal = document.createElement("fluent-dialog");
  modal.style.setProperty("--dialog-height", "200px");
  modal.style.setProperty("z-index", "1000");
  modal.setAttribute("trap-focus", "");
  ...
  <div style="display: flex; justify-content: space-between; margin-top: 0.4rem;">
    <fluent-button appearance="accent" onclick="promptAction(true, this.closest('fluent-dialog'), ${callback}, ${args})" style="width: 100%">确定</fluent-button>
    <fluent-button onclick="promptAction(false, this.closest('fluent-dialog'), ${callback}, ${args})" style="width: 100%;margin-left: 5px;">取消</fluent-button>
  </div>
  </div>`;
  document.body.appendChild(modal);
  modal.hidden = false;
  ...
  modal.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      promptAction(true, modal, callback, args);
    } else if (event.key === "Escape") {
      promptAction(false, modal, callback, args);
    }
  });
}

function promptAction(result, modal, callback, args) {
  const input = document.getElementById("prompt-input").value;
  modal.hidden = true;
  document.body.removeChild(modal);
  callback(result ? input : null, args);
}
